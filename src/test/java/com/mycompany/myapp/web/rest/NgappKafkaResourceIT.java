...
class NgappKafkaResourceIT {
    ...
    void setup() {
        ...
        NgappKafkaResource kafkaResource = new NgappKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/ngapp-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/ngapp-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
